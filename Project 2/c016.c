...
void htInit ( tHTable* ptrht ) {
	...
	for (int i=0;i<MAX_HTSIZE;i++){		//cyklus nastavý kazdy prvok tabulky na NULL
			(*ptrht)[i]=NULL;
		}
return;
}
...
void htInsert ( tHTable* ptrht, tKey key, tData data ) {
	...
	if (htSearch(ptrht,key)){						//ak je item najdeny aktuelizuje data
 		htSearch(ptrht,key)->data=data;
 	}
 	else{
 		...
 		Ninsert->data=data;							//nahraju sa data
...
void htClearAll ( tHTable* ptrht ) {
	...
	for (int i=0;i<MAX_HTSIZE;i++){	//cyklus prechadza tabulkov
