...
*/
	...
	if (q == NULL){								//Ak je q ukazuje na NULL tak sa zavola queueError(QERR_INIT)
		
		queueError(QERR_INIT);
	}
	else{										//Inak naplni pole znakmi '*' a vynuluje indexi
...
return ((index + 1) % QUEUE_SIZE); // Modulo zabezpecuje ze index nepresiahne pocet prvkov
}
...
return q->f_index == q->b_index; 	// Ak indexi ukazuju na rovnaku hodnotu tak je zoznam prazdny
}
...
return nextIndex(q->b_index) == q->f_index;	// Ak je b_indexe posledny tak vrati nenulovu hodnotu  
}
...
*/
	...
	if (queueEmpty(q)){				//viuziva queueEmpty() vid. vyssie ak je true vracia chybu

		queueError(QERR_FRONT);
	}
	else{				//vracia prvy prvok fronty
...
*/
	...
	if (queueEmpty(q)){						//Vyuziva queueEmpty() vid. vyssie ak je true vracia chybu

		queueError(QERR_REMOVE);
	}
	else{									//Posunie zaciatok fronty na druhy prvok
...
*/
	...
	if (queueEmpty(q)){					//Vyuziva queueEmpty() vid. vyssie ak je true vracia chybu
		...
		queueFront(q, c);	//Ulozi prvy prvok do c
		queueRemove(q); 	//A posunie frontu
	}
return;
}
...
*/
	...
	if (queueFull(q)){				//Vyuziva queueFull() vid. vyssie ak je true vracia chybu
		...
		q->arr[q->b_index] = c;		// Vlozi prvok c na miesto, ktore oznacuje b_index
		q->b_index = nextIndex(q->b_index); //posunie b_index vpravo
	}
return;
}
